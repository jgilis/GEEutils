...
.bakery_KC <- function(gee.fit){
    ...
    for (i in 1:size) {
      ...
      step13 <- step13 + matrixcalc::vec(xy %*% t(xy)) # could change matrixcalc::vec to c
...
#'
#' @description Compute robust standard error estimates using multiple sandwich strategies
#'
#' @param formula A formula expression as for other regression models, of the form response ~ predictors.
#'  See the documentation of `lm` and `formula` for details.
#'
#' @param id A `vector` which identifies the clusters. The length of id should be the same as the number of observations.
...
#'
#' @param family A `character string` indicating the family for defining link and variance functions.
#'  Currently, only "poisson" is supported.
#'
#' @param corstr A `character string` specifying the correlation structure. Default is "exchangeable".
#'  The following are permitted: "independence", "fixed", "stat_M_dep", "non_stat_M_dep", "exchangeable", "AR-M" and "unstructured".
#'
#' @param silent A `logical` variable controlling whether parameter estimates at each iteration are printed.
#'
#' @param extraSandwich A `character vector` of sandwich estimation procedures that must be used to compute robust standard errors,
...
bakery <- function(formula, id, data, family, corstr, silent = TRUE, extraSandwich = "none"){
