...
#'
#' @export
filterBySample <- function(object, group_id, sample_id,
                           size = 2, assay = NULL) {
    if (is.null(assay)) {
...
.split_cells_filter <- function(x, by) {
    ...
    cd <- split(cd, by = by, sorted = TRUE, flatten = FALSE, drop = TRUE)
...
.pb_filter <- function(x, cs, assay) {
    ...
    map_depth(pb, -2, function(u) {
        as.matrix(data.frame(u, row.names = rownames(x), check.names = FALSE))
    })
}
