...
.getSumStats <- function(object, clustering){
    ...
    computemeans <- function(x){tapply(x, object[[clustering]], function(a){mean(a[a != 0])})}
    temp.intrameans <- apply(counts, 1, computemeans)

    computedrop <- function(x){tapply(x,object[[clustering]],function(a){length(a[which(a == 0)])/length(a)})}
    ...
    sumStats <- as.data.frame(t(rbind(temp.intrameans,temp.intravar,temp.drop)))
    colnames(sumStats) <- paste0(rep(c("Mean_","Var_","Drop_"), each = nclust), colnames(sumStats))

    sumStats$InterStD <- apply(sumStats[,1:nclust], 1, function(a){stats::sd(a[!is.na(a)])})
    sumStats$GrandMean <- apply(sumStats[,1:nclust], 1, function(a){mean(a[!is.na(a)])})
    sumStats$IntraVar <- apply(sumStats[,(1+nclust):(nclust*2)], 1, median)
    sumStats$DropOut <- apply(sumStats[,(1+(nclust*2)):(nclust*3)], 1, mean)
    sumStats$DropOutStD <- apply(sumStats[,(1+(nclust*2)):(nclust*3)], 1, function(a){stats::sd(a[!is.na(a)])})
...
.getHyperDropout <- function(sumStats, plot){
    ...
    if(plot){
      gg <- ggplot(sumStats, aes(DropOut)) +
        ...
        ylab("Density")  +
        stat_function(fun=dgamma, args=list(shape=drop.shape, rate=drop.rate)) +
        theme_bw()
      print(gg)
    }
    ...
    dropoutParams <- list(unname(drop.shape), unname(drop.rate), dropoutstd.beta0, dropoutstd.beta1, dropoutstd.beta2)
    names(dropoutParams) <- c("drop.shape", "drop.rate", "drop.beta0", "drop.beta1", "drop.beta2")
...
.getHyperGrandmean <- function(sumStats, plot){
    ...
    if(plot){
      gg <- ggplot(sumStats, aes(GrandMean)) +
        geom_histogram(aes(y=..density..),fill="cornflowerblue",color = "black") +
        ylab("Density") +
        stat_function(fun=dgamma, args=list(shape=grandmean.shape, rate=grandmean.rate)) +
        ggtitle("Grandmean") +
        theme_bw()
      print(gg)
...
.getHyperRelationship1 <- function(sumStats, plot){
    ...
    temp.mean.inter <- glm(data = sumStats, InterStD ~ 0 + GrandMean, family = gaussian(link = "identity"))
    inter.beta1 <- summary(temp.mean.inter)$coefficients[1,1]

    if(plot){
      gg <- ggplot(sumStats,aes(x=GrandMean,y=InterStD)) +
        geom_point() +
        stat_smooth(method="glm",formula = y ~ 0 + x, method.args = list(family = gaussian(link = "identity")), size = 1.6) +
...
.getHyperRelationship2 <- function(sumStats, plot){
    ...
    intravar <- na.omit(as.data.frame(cbind(unlist(sumStats[,which(grepl("Mean_",colnames(sumStats)))]),
    ...
    if(plot){
      gg <- ggplot(intravar,aes(x=IntraMean,y=Dispersion)) +
        geom_point() +
        stat_smooth(method="glm",formula = y ~ I(1/x), method.args = list(family = gaussian(link = "log")), size = 1.6) +
    ...
    return(relationship2Params)
}

#' Obtain parameters for simulating synthetic asRNA-Seq data based on a real scRNA-Seq dataset.
#'
#' @description Extract parameters from a scRNA-Seq dataset that are relevant for simulating synthetic asRNA-Seq data.
#'
...
#'
#' @param plot `Logical` TRUE or FALSE; should intermediate visualizations be generated?
#'
...
getSimulationParameters <- function(object, clustering, plot=TRUE, verbose=FALSE){
    ...
    if(verbose) message("Hyperparameters realtionship 1")
    relationship1Params <- .getHyperRelationship1(sumStats, plot)

    if(verbose) message("Hyperparameters realtionship 2")
    relationship2Params <- .getHyperRelationship2(sumStats, plot)

    if(verbose) message("Combine all hyperparameters")
    SimulationParameters <- c(dropoutParams, grandmeanParams, relationship1Params, relationship2Params)
