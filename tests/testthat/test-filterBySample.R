...
ref$subject_id <- rep(paste0("patient", 1:8), each = ncol(ref) / 8)

test_that("filterBySample() works", {
    n_drop <- 10L # number of genes that should be removed

    ## Set some genes to 0 for all but 1 patient
    counts(ref)[seq_len(n_drop), -which(ref$subject_id == "patient1")] <- 0
    to_drop <- rownames(ref)[seq_len(n_drop)]

    out <- filterBySample(ref, sample_id = "subject_id", size = 2)
