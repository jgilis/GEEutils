...
- `empirical_simulator`: similar to `simulator`, but obtains the parameters for
  the `grandMean` and `dropOut` empirically, i.e. by sampling from the observed
...
```{r}
gg <- GEEutils::correlationsBoxplot(
    object = sceNoCor,
    clustering = "patient_id"
)
gg
...
```{r}
SimulationParameters <- GEEutils::getSimulationParameters(
    object = sceNoCor,
    clustering = "patient_id",
    plot = TRUE,
    verbose = TRUE
...
# could also do this on the sce that still has correlated genes
...
simulated_pois <- GEEutils::simulator(
    design = design,
...
simulated_pois[1:5, 1:5]

# set.seed(55)
# simulated_NB <- GEEutils::simulator(design = design,
#                               SimulationParameters = SimulationParameters,
#                               nGenes = nGenes,
#                               distribution = distribution,
#                               fractionDGE = fractionDGE,
#                               foldchange = foldchange)
#
# simulated_NB[1:5,1:5]
#
# sum(is.na(simulated_NB))
# simulated_NB[is.na(simulated_NB)] <- 0 # not nice, resolve later
```

# Simulate new data (empirical)

As opposed to the chunk above, we here use the `empirical_simulator` function
which  obtains the parameters for the `grandMean` and `dropOut` empirically, 
i.e. by sampling from the observed empirical distributions.

```{r}
# simulate from poisson
emp_simulated_pois <- GEEutils::empirical_simulator(
    object = sceNoCor,
    clustering = "patient_id",
    design = design,
    SimulationParameters = SimulationParameters,
...
dds3 <- DESeqDataSetFromMatrix(
    countData = simulated_NB,
    colData = designCell,
    design = ~cluster
)

dds4 <- DESeqDataSetFromMatrix(
    countData = simulated_pois,
    colData = designCell,
    design = ~cluster
)
...
names(dds_list) <- c("Kang_CD14_filtered", "Kang_CD14_filtered_noCor", "Simulated_NB", "Simulated_pois")
